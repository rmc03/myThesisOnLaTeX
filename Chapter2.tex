...
El público objetivo abarca usuarios de todas las edades, con especial énfasis en el contexto escolar cubano, donde la disponibilidad de materiales didácticos digitales actualizados sobre los ODS es aún escasa. El diseño del videojuego respeta los colores, íconos y denominaciones oficiales de cada ODS, lo que refuerza la identificación visual del contenido educativo y facilita su uso como complemento de programas curriculares relacionados con la sostenibilidad.
...
El flujo de la partida es coordinado secuencialmente por el \texttt{GameManager}. Al inicio, se procesa la configuración de la partida y se instancian los jugadores en la posición cero. Un turno arranca cuando el jugador activo detona el lanzamiento del dado obteniendo un valor de uno a seis, desplazando su ficha hacia el destino mediante una animación interpolada fluida.
...
Para garantizar la integridad del sistema frente a errores, se implementó una estrategia de resiliencia estructurada: si el sistema detecta que falta el archivo JSON de preguntas en la ruta esperada, emite una advertencia en la consola de depuración y automáticamente carga un conjunto de preguntas de respaldo codificadas internamente, asegurando que el ciclo de juego no se interrumpa. De igual forma, al procesar el JSON, el \texttt{QuestionManager} valida la estructura de cada diccionario entrante; si una entrada está malformada, por ejemplo al carecer de la clave obligatoria \texttt{answer} u \texttt{options}, el sistema descarta esa pregunta específica de la cola de selección aleatoria y registra un error de validación, previniendo excepciones fatales durante el turno del jugador y manteniendo la fluidez de la partida incluso ante datos corruptos. Finalmente, la separación del \texttt{GameState} garantiza que, ante una interrupción abrupta de la partida, las estadísticas temporales no contaminen los registros históricos de victorias.

\subsection{Tareas de ingeniería}
\label{subsec:tareas_ingenieria}

Las tareas de ingeniería son los componentes técnicos específicos derivados de las historias de usuario. Representan el trabajo concreto que debe realizarse para implementar cada funcionalidad. A continuación se expone de manera representativa, en la Tabla \ref{tab:ti5}, la tarea de ingeniería responsable del manejo interactivo de los ODS, vinculada a la HU7 explicada anteriormente. El conjunto completo con las diez tareas de ingeniería organizadas por iteración se encuentra disponible en el Anexo \ref{anx:tareas_ingenieria}.
